...
const ManagerTimesheets = () => {
  ...
  const handleApprove = async (id) => {
    setLoading(true);
    try {
      await axios.put(`http://localhost:8080/api/timesheets/Approve/${id}/status/APPROVED`);
      fetchSubmissions();
    } catch (error) {
  ...
  const handleReject = async () => {
    setLoading(true);
    try {
      await axios.put(`http://localhost:8080/api/timesheets/reject/${currentId}/status/REJECTED/comments/${comments}`);
      fetchSubmissions();
      handleClose();
    } catch (error) {
